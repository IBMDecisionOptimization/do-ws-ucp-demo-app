...
inputs = {};
outputs = {};
inputs["items"] = pd.read_csv("items.csv")
inputs["parameters"] = pd.read_csv("parameters.csv")
inputs["prices"] = pd.read_csv("prices.csv")
...
total_order_cost = sum( order_qty[item,day] * prices.price[item,day] for day in all_days for item in all_items)    
total_stock_cost = sum( stock_qty[item,day] * items.stockCost[item] for day in all_days for item in all_items)    
nb_orders = sum( do_order[day] for day in all_days )    

mdl.add_kpi(total_order_cost, "Total Order Cost")
mdl.add_kpi(total_stock_cost, "Total Stock Cost")
mdl.add_kpi(nb_orders, "Number of Orders")

mdl.minimize(total_stock_cost + total_order_cost)
